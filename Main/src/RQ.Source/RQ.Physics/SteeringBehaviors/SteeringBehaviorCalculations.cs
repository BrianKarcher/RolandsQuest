...
namespace RQ.Physics.SteeringBehaviors
{
    ...
    public static class SteeringBehaviorCalculations
    {
        //------------------------------- Seek -----------------------------------
        //
        ...
        //------------------------------------------------------------------------
        public static Vector2D Seek(Vector2 TargetPos, IBasicPhysicsComponent entity)
        {
            var physicsAffector = entity.GetSteering().SteeringPhysicsAffector;
            var DesiredVelocity = (TargetPos - entity.GetFeetWorldPosition()).normalized
                                    * physicsAffector.MaxSpeed;
        ...
        //------------------------------------------------------------------------
        public static Vector2 Flee(Vector2 TargetPos, IBasicPhysicsComponent entity)
        {
            var physicsAffector = entity.GetSteering().SteeringPhysicsAffector;
            ...
            var DesiredVelocity = (entity.GetWorldPos2D() - TargetPos).normalized
                                      * physicsAffector.MaxSpeed;
        ...
        //------------------------------------------------------------------------
        public static Vector2D Arrive(Vector2 TargetPos, IBasicPhysicsComponent entity,
                                          Deceleration deceleration)
        {
            var physicsAffector = entity.GetSteering().SteeringPhysicsAffector;
            var ToTarget = TargetPos - entity.GetFeetWorldPosition();
            ...
            if (dist > 0)
            {
                ...
                speed = Mathf.Min(speed, physicsAffector.MaxSpeed);
        ...
        //  evader
        //------------------------------------------------------------------------
        public static Vector2D Pursuit(IBasicPhysicsComponent evader, IBasicPhysicsComponent entity)
        {
            var physicsAffector = entity.GetSteering().SteeringPhysicsAffector;
            //if the evader is ahead and facing the agent then we can just seek
            //for the evader's current position.
            var ToEvader = evader.GetWorldPos2D() - entity.GetWorldPos2D();

            float RelativeHeading = Vector2.Dot(entity.GetPhysicsData().Heading, evader.GetPhysicsData().Heading);

            if (Vector2.Dot(ToEvader, entity.GetPhysicsData().Heading) > 0 &&
                 (RelativeHeading < -0.95f))  //acos(0.95)=18 degs
            {
                return Seek(evader.GetWorldPos(), entity);
            }
            ...
            float LookAheadTime = ToEvader.magnitude /
                                  (physicsAffector.MaxSpeed + evader.GetVelocity().magnitude);

            //now seek to the predicted future position of the evader
            return Seek(evader.GetFeetWorldPosition() + evader.GetVelocity() * LookAheadTime, entity);
        }
        ...
        //------------------------------------------------------------------------
        public static Vector2D Evade(IBasicPhysicsComponent pursuer, IBasicPhysicsComponent entity)
        {
            var physicsAffector = entity.GetSteering().SteeringPhysicsAffector;
            /* Not necessary to include the check for facing direction this time */

            Vector2D ToPursuer = pursuer.GetWorldPos() - entity.GetWorldPos();

            //uncomment the following two lines to have Evade only consider pursuers 
            //within a 'threat range'
            const float ThreatRange = 100.0f;
            if (ToPursuer.LengthSq() > ThreatRange * ThreatRange) return Vector2D.Zero();

            //the lookahead time is propotional to the distance between the pursuer
            //and the pursuer; and is inversely proportional to the sum of the
            //agents' velocities
            float LookAheadTime = ToPursuer.Length() /
                                   (physicsAffector.MaxSpeed + pursuer.GetVelocity().magnitude);

            //now flee away from predicted future position of the pursuer
            return Flee(pursuer.GetFeetWorldPosition() + pursuer.GetVelocity() * LookAheadTime, entity);
        }
        ...
        public static Vector2 RadiusClamp(IBasicPhysicsComponent entity)
        {
            var data = entity.GetPhysicsData();
            var target = entity.GetSteering().GetTarget();
            // Estimate position entity will be in, in one second
            var projectedNearFutureLocation = ((Vector2)entity.transform.position) + entity.GetVelocity();
            ...
            float wanderRadius, float wanderDistance, Transform transform, ref Vector2D last_wander_pos)
        {
            ...
            var deltaWanderTarget = new Vector2D(RandomClamped() * JitterThisTimeSlice,
                                        RandomClamped() * JitterThisTimeSlice);
            ...
            if (entity.GetPhysicsData().Heading == Vector2.zero)
            {
            ...
            //and steer towards it
            var physicsAffector = entity.GetSteering().SteeringPhysicsAffector;
            var force = Target - entity.GetFeetWorldPosition();
            force = force.normalized * physicsAffector.MaxSpeed;
            return force;
        ...
        //Vector2D SteeringBehavior::ObstacleAvoidance(const std::vector<BaseGameEntity*>& obstacles)
        {
            ...
            var m_dDBoxLength = MinDetectionBoxLength +
                            (entity.GetVelocity().magnitude / physicsAffector.MaxSpeed) *
            ...
            Vector2D LocalPosOfClosestObstacle = Vector2D.Zero();
            //    // 1 << 10 is the layer for Enemies.
            //       12 = Bounding Radius
            int layerMask = 1 << 12;
            var rays = UnityEngine.Physics.BoxCastAll(entity.transform.position, new Vector3(0.16f, 0.16f, 0f), 
                entity.GetPhysicsData().Heading, Quaternion.identity, m_dDBoxLength, layerMask);
            //if (UnityEngine.Physics.BoxCast(entity.transform.position, new Vector3(0.16f, 0.16f, 0f), direction.ToVector3(0f),
            //    out hit, Quaternion.identity, castDistance, layerMask))
            ...
            {
                var otherCollisionComponent = ray.collider.transform.GetComponent<CollisionComponent>();
                ...
                if (otherEntity == entity)
                    continue;
                var otherPhysicsComponent = otherEntity.Components.GetComponent<IBasicPhysicsComponent>();
                //Closest
            ...
            if (ClosestIntersectingObstacle != null)
            {
                ...
                var otherPhysicsComponent = ClosestIntersectingObstacle.Components.GetComponent<IBasicPhysicsComponent>();
        ...
        */
        ...
        //------------------------------------------------------------------------
        public static Vector2D Interpose(IBasicPhysicsComponent AgentA,
                                             IBasicPhysicsComponent AgentB,
                                                IBasicPhysicsComponent entity)
        {
            var physicsAffector = entity.GetSteering().SteeringPhysicsAffector;
            //first we need to figure out where the two agents are going to be at 
            //time T in the future. This is approximated by determining the time
            //taken to reach the mid way point at the current time at at max speed.
            Vector2D MidPoint = (AgentA.GetWorldPos() + AgentB.GetWorldPos()) / 2.0f;

            float TimeToReachMidPoint = Vector2D.Vec2DDistance(entity.GetWorldPos(), MidPoint) /
                                         physicsAffector.MaxSpeed;

            //now we have T, we assume that agent A and agent B will continue on a
            //straight trajectory and extrapolate to get their future positions
            Vector2D APos = AgentA.GetWorldPos2D() + AgentA.GetVelocity() * TimeToReachMidPoint;
            Vector2D BPos = AgentB.GetWorldPos2D() + AgentB.GetVelocity() * TimeToReachMidPoint;

            //calculate the mid point of these predicted positions
            MidPoint = (APos + BPos) / 2.0f;

            //then steer to Arrive at it
            return Arrive(MidPoint, entity, Deceleration.fast);
        }
        ...
        //------------------------------------------------------------------------
        public static Vector2D FollowPath(IBasicPhysicsComponent entity, Path path)
        {
            //if (path.IsFinished())
        ...
        //------------------------------------------------------------------------
        public static Vector2D OffsetPursuit(IBasicPhysicsComponent leader,
                                                  Vector2 offset, IBasicPhysicsComponent entity)
        {
            var physicsAffector = entity.GetSteering().SteeringPhysicsAffector;
            ...
            var WorldOffsetPos = leader.GetWorldPos2D() + offset;
