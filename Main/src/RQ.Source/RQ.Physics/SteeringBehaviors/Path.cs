...
namespace RQ.AI
{
    public class Path
    {
        ...
        public Path()
        {
            ...
            _pathType = Physics.Pathfinding.PathType.Looped;
        ...
        {
            if (_pathType != Physics.Pathfinding.PathType.Once)
                return false;

            if (_pathWalkingDirection == Physics.SteeringBehaviors.PathWalkingDirection.Forwards)
                return _curWaypoint == _wayPoints.Count - 1;
            if (_pathWalkingDirection == Physics.SteeringBehaviors.PathWalkingDirection.Backwards)
        ...
        public void SetNextWaypoint()
        {
            ...
            if (_pathWalkingDirection == PathWalkingDirection.Forwards && _curWaypoint == lastWaypoint)
                {
                    if (_pathType == Physics.Pathfinding.PathType.Looped)
                    {
                        _curWaypoint = 0;
                        return;
                    }
                    else if (_pathType == Physics.Pathfinding.PathType.Pingpong)
                    {
                        _pathWalkingDirection = Physics.SteeringBehaviors.PathWalkingDirection.Backwards;
                    }
                    else
                    {
                        _isFinished = true;
            ...
            if (_pathWalkingDirection == PathWalkingDirection.Backwards && _curWaypoint == 0)
                {
                    ...
                    {
                        _pathWalkingDirection = Physics.SteeringBehaviors.PathWalkingDirection.Forwards;
                    }
                    else
                    {
                        _isFinished = true;
