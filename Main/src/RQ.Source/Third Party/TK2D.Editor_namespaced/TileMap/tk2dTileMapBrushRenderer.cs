		...
		void BuildMeshForBrush(tk2dTileMapEditorBrush brush, BrushDictData dictData, int tilesPerRow)
		{
			...
			else
			{
				...
				foreach (var tile in brush.tiles)
				{
					...
					int spriteIdx = tk2dRuntime2.TileMap.BuilderUtil.GetTileFromRawTile(tile.spriteId);
					bool flipH = tk2dRuntime2.TileMap.BuilderUtil.IsRawTileFlagSet(tile.spriteId, tk2dTileFlags.FlipX);
					bool flipV = tk2dRuntime2.TileMap.BuilderUtil.IsRawTileFlagSet(tile.spriteId, tk2dTileFlags.FlipY);
					bool rot90 = tk2dRuntime2.TileMap.BuilderUtil.IsRawTileFlagSet(tile.spriteId, tk2dTileFlags.Rot90);
					...
					for (int j = 0; j < sprite.positions.Length; ++j)
					{
						Vector3 flippedPos = tk2dRuntime2.TileMap.BuilderUtil.ApplySpriteVertexTileFlags(tileMap, sprite, sprite.positions[j], flipH, flipV, rot90);
