...
namespace tk2dEditor2
{
    ...
    public class tk2dTileMapEditor : Editor, ITileMapEditorHost
    {
        ...
        tk2dEditor2.BrushRenderer _brushRenderer;
        tk2dEditor2.BrushRenderer brushRenderer
        {
            get
            {
                if (_brushRenderer == null) _brushRenderer = new tk2dEditor2.BrushRenderer(tileMap);
        ...
        tk2dEditor2.BrushBuilder _guiBrushBuilder;
        tk2dEditor2.BrushBuilder guiBrushBuilder
        {
            get
            {
                if (_guiBrushBuilder == null) _guiBrushBuilder = new tk2dEditor2.BrushBuilder();
        ...
        void DrawLayersPanel(bool allowEditing)
        {
            ...
            if (allowEditing)
            {
                ...
                {
                    editorData.layer = tk2dEditor2.TileMap.TileMapUtility.AddNewLayer(tileMap);
                }
            ...
            if (deleteLayer != -1)
            {
                ...
                tk2dEditor2.TileMap.TileMapUtility.DeleteLayer(tileMap, deleteLayer);
            }

            if (moveUp != -1)
            {
                //Undo.RegisterUndo(new Object[] { tileMap, tileMap.data }, "Moved layer");
                tk2dEditor2.TileMap.TileMapUtility.MoveLayer(tileMap, moveUp, -1);
            }

            if (moveDown != -1)
            {
                //Undo.RegisterUndo(new Object[] { tileMap, tileMap.data }, "Moved layer");
                tk2dEditor2.TileMap.TileMapUtility.MoveLayer(tileMap, moveDown, 1);
            }
        ...
        void DrawSettingsPanel()
        {
            ...
            {
                tk2dEditor2.TileMap.TileMapUtility.MakeUnique(tileMap);
            }
            GUILayout.EndHorizontal();
            GUILayout.Space(8);
            ...
            if (Foldout(ref editorData.setupMode, tk2dTileMapEditorData.SetupMode.Dimensions, "Dimensions"))
            {
                ...
                width = Mathf.Clamp(EditorGUILayout.IntField("Width", width), 1, tk2dEditor2.TileMap.TileMapUtility.MaxWidth);
                height = Mathf.Clamp(EditorGUILayout.IntField("Height", height), 1, tk2dEditor2.TileMap.TileMapUtility.MaxHeight);
                ...
                {
                    tk2dRuntime2.TileMap.BuilderUtil.InitDataStore(tileMap);
                    tk2dEditor2.TileMap.TileMapUtility.ResizeTileMap(tileMap, width, height, tileMap.partitionSizeX, tileMap.partitionSizeY);
                }
                ...
                if (width != tileMap.width || height != tileMap.height || partitionSizeX != tileMap.partitionSizeX || partitionSizeY != tileMap.partitionSizeY)
                {
                    ...
                    {
                        tk2dEditor2.TileMap.TileMapUtility.ResizeTileMap(tileMap, width, height, partitionSizeX, partitionSizeY);
                    }
                    GUILayout.EndHorizontal();
                }
            ...
            if (Foldout(ref editorData.setupMode, tk2dTileMapEditorData.SetupMode.Import, "Import"))
            {
                ...
                {
                    if (tk2dEditor2.TileMap.Importer.Import(tileMap, tk2dEditor2.TileMap.Importer.Format.TMX))
                    {
                        Build(true);
                        width = tileMap.width;
                        height = tileMap.height;
		...
		DrawLoadSaveBrushSection(activeBrush);
            ...
            if (!showLoadSection && !showSaveSection)
            {
                ...
                if (editorData.showPalette)
                {
                    ...
                    {
                        int tile = tk2dRuntime2.TileMap.BuilderUtil.GetTileFromRawTile(activeBrush.tiles[0].spriteId);
        ...
        public override void OnInspectorGUI()
        {
            ...
            else
            {
                ...
                if (tk2dRuntime2.TileMap.BuilderUtil.InitDataStore(tileMap))
                    Build(true);
