...
namespace tk2dEditor2.SpriteCollectionBuilder
{
	public static class PlatformBuilder
	{
		...
		{
			tk2dEditor2.SpriteCollectionEditor.SpriteCollectionProxy proxy = new tk2dEditor2.SpriteCollectionEditor.SpriteCollectionProxy(source);
			...
			for (int i = 0; i < proxy.fonts.Count; ++i)
			{
				...
				if (!root)
				{
					...
					if (bmFontPath.Length != 0 && texturePath.Length == 0)
					{
						...
						tk2dEditor2.Font.Info fontInfo = tk2dEditor2.Font.Builder.ParseBMFont(bmFontPath);
						if (fontInfo != null)
