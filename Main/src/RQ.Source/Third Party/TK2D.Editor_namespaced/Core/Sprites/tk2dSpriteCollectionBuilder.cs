...
namespace tk2dEditor2
{
 ...
 importer.maxTextureSize < 4096)
            ...
            if (numNotTransparent > 0)
            {
                ...
                switch (textureCompression)
                {
                    case tk2dSpriteCollection.TextureCompression.Dithered16Bit_NoAlpha:
                        tk2dEditor2.TextureProcessing.FloydSteinbergDithering.DitherTexture(dtex, TextureFormat.ARGB4444, 0, 0, dtex.width, dtex.height); break;
                    case tk2dSpriteCollection.TextureCompression.Dithered16Bit_Alpha:
                        tk2dEditor2.TextureProcessing.FloydSteinbergDithering.DitherTexture(dtex, TextureFormat.RGB565, 0, 0, dtex.width, dtex.height); break;
                }
                dtex.Apply();
        ...
        public static bool Rebuild(tk2dSpriteCollection gen)
        {
            ...
            if (gen.version < 2)
            {
                if (!tk2dEditor2.SpriteCollectionBuilder.Deprecated.CheckAndFixUpParams(gen))
                {
                    // Params failed check
                    return false;
                }
                tk2dEditor2.SpriteCollectionBuilder.Deprecated.SetUpSpriteSheets(gen);
                tk2dEditor2.SpriteCollectionBuilder.Deprecated.TrimTextureList(gen);
            }
            currentBuild = gen;
            ...
            if (gen.HasPlatformData)
            {
                ...
                tk2dEditor2.SpriteCollectionBuilder.PlatformBuilder.InitializeSpriteCollectionPlatforms(gen, System.IO.Path.GetDirectoryName(prefabObjectPath));
                ...
                for (int i = 0; i < gen.platforms.Count; ++i)
                {
                    ...
                    if (validPlatform)
                    {
                        ...
                        tk2dEditor2.SpriteCollectionBuilder.PlatformBuilder.UpdatePlatformSpriteCollection(
            ...
            tk2dEditor2.SpriteCollectionBuilder.LinkBuilder.ValidateLinkedSpriteCollection(gen);
            ...
            for (int i = 0; i < gen.textureParams.Length; ++i)
            {
                ...
                tk2dEditor2.SpriteCollectionBuilder.LinkBuilder.ValidateTextureParam(gen, i);
            ...
            // Font
            Dictionary<tk2dSpriteCollectionFont, tk2dEditor2.Font.Info> fontInfoDict = new Dictionary<tk2dSpriteCollectionFont, tk2dEditor2.Font.Info>();
            if (gen.fonts != null)
            {
                for (int i = 0; i < gen.fonts.Length; ++i)
                {
                    ...
                    tk2dEditor2.Font.Info fontInfo = tk2dEditor2.Font.Builder.ParseBMFont(AssetDatabase.GetAssetPath(font.bmFont));
            ...
            tk2dEditor2.Atlas.Builder atlasBuilder = new tk2dEditor2.Atlas.Builder(atlasWidth, atlasHeight, gen.allowMultipleAtlases ? 64 : 1, allowFindingOptimalSize, forceSquareAtlas, allowRotation);
            ...
            tk2dEditor2.Atlas.Data[] atlasData = atlasBuilder.GetAtlasData();
            ...
            tk2dEditor2.SpriteCollectionBuilder.LinkBuilder.Build(gen);
                    ...
                    foreach (var type in types)
                    {
                        if (type.GetInterface("tk2dRuntime.ISpriteCollectionForceBuild") != null)
                        {
                            ...
                            foreach (var o in objects)
                            {
                                ...
                                tk2dRuntime2.ISpriteCollectionForceBuild isc = o as tk2dRuntime2.ISpriteCollectionForceBuild;
        ...
        static void UpdateFontData(tk2dSpriteCollection gen, float scale, tk2dEditor2.Atlas.Data[] packers, List<SpriteLut> spriteLuts, tk2dSpriteCollectionFont font, tk2dEditor2.Font.Info fontInfo)
        {
            ...
            tk2dEditor2.Font.Builder.BuildFont(fontInfo, font.data, scale, font.charPadX, font.dupeCaps, font.flipTextureY, font.gradientTexture, font.gradientCount);
        }

        static void UpdateVertexCache(tk2dSpriteCollection gen, float scale, tk2dEditor2.Atlas.Data[] packers, tk2dSpriteCollectionData coll, List<SpriteLut> spriteLuts)
        {
            for (int i = 0; i < sourceTextures.Length; ++i)
            {
                ...
                tk2dEditor2.Atlas.Data packer = packers[0];
                tk2dEditor2.Atlas.Entry atlasEntry = null;
                int atlasIndex = 0;
                if (_lut != null)
                {
                ...
                else
                {
                    ...
                    else if (thisTexParam.customSpriteGeometry)
                    {
                        ...
                        foreach (var island in thisTexParam.geometryIslands)
                        {
                            ...
                            tk2dEditor2.Triangulator triangulator = new tk2dEditor2.Triangulator(island.points);
        ...
        static void UpdateColliderData(tk2dSpriteCollection gen, float scale, tk2dSpriteCollectionData coll, int spriteIndex, Vector3 origin)
        {
            ...
            else if (colliderType == tk2dSpriteCollectionDefinition.ColliderType.Polygon)
            {
                ...
                foreach (var island in src.polyColliderIslands)
                {
                    ...
                    {
                        tk2dEditor2.Triangulator triangulator = new tk2dEditor2.Triangulator(points2D.ToArray());
