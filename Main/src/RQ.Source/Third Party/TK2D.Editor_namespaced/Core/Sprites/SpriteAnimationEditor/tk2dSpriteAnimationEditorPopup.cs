...
namespace tk2dEditor2
{
    public class tk2dSpriteAnimationEditorPopup : EditorWindow
    {
        ...
        tk2dEditor2.SpriteAnimationEditor.ClipEditor clipEditor = null;

        tk2dEditor2.SpriteAnimationEditor.AnimOperator[] animOps = new tk2dEditor2.SpriteAnimationEditor.AnimOperator[0];

        // "Create" menu
        GUIContent[] menuItems = new GUIContent[0];
        tk2dEditor2.SpriteAnimationEditor.AnimOperator[] menuTargets = new tk2dEditor2.SpriteAnimationEditor.AnimOperator[0];

        void OnEnable()
        {
            // Detect animOps
            List<tk2dEditor2.SpriteAnimationEditor.AnimOperator> animOpList = new List<tk2dEditor2.SpriteAnimationEditor.AnimOperator>();
            foreach (var assembly in System.AppDomain.CurrentDomain.GetAssemblies())
            {
                try
                {
                    ...
                    {
                        if (type.BaseType == typeof(tk2dEditor2.SpriteAnimationEditor.AnimOperator))
                        {
                            tk2dEditor2.SpriteAnimationEditor.AnimOperator inst = (tk2dEditor2.SpriteAnimationEditor.AnimOperator)System.Activator.CreateInstance(type);
                            if (inst != null)
                                animOpList.Add(inst);
                        }
                    }
            ...
            List<tk2dEditor2.SpriteAnimationEditor.AnimOperator> menuTargets = new List<tk2dEditor2.SpriteAnimationEditor.AnimOperator>();
            menuItems.Add(new GUIContent("Clip")); menuTargets.Add(null);

            foreach (tk2dEditor2.SpriteAnimationEditor.AnimOperator animOp in animOps)
            {
            ...
            if (clipEditor == null)
            {
                clipEditor = new tk2dEditor2.SpriteAnimationEditor.ClipEditor();
        ...
        void FilterClips()
        {
            ...
            if (searchFilter.Length == 0)
            {
                filteredClips = (from clip in allClips where !clip.Empty select clip)
                                .OrderBy(a => a.name, new tk2dEditor2.Shared.NaturalComparer())
                                .ToList();
            }
            else
            {
        ...
        {
            // Find a unique name
            string uniqueName = tk2dEditor2.SpriteAnimationEditor.AnimOperatorUtil.UniqueClipName(allClips, "New Clip");
            ...
            if (GUI.Button(createButtonRect, createButton, EditorStyles.toolbarDropDown) && anim != null)
            {
                ...
                EditorUtility.DisplayCustomMenu(createButtonRect, menuItems, -1,
                    delegate(object userData, string[] options, int selected)
                    {
                        ...
                        {
                            tk2dEditor2.SpriteAnimationEditor.AnimOperator animOp = menuTargets[selected];
                            ...
                            if ((animOp.AnimEditOperations & tk2dEditor2.SpriteAnimationEditor.AnimEditOperations.AllClipsChanged) != tk2dEditor2.SpriteAnimationEditor.AnimEditOperations.None)
                            {
                                FilterClips();
                                Repaint();
                            }
                            if ((animOp.AnimEditOperations & tk2dEditor2.SpriteAnimationEditor.AnimEditOperations.NewClipCreated) != tk2dEditor2.SpriteAnimationEditor.AnimEditOperations.None)
                            {
                                clipEditor.InitForNewClip();
                                Repaint();
                            }
