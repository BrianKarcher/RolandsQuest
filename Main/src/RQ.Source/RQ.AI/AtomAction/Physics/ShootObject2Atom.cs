...
namespace RQ.AI.AtomAction.Physics
{
    public enum ShootTarget
    {
        ...
        Random = 2,
        ToLocation = 3,
        ToParentFacingDirection = 4
    }

    /// <summary>
    /// Shoots a supplied Game Object
    /// </summary>
    public class ShootObject2Atom : AtomActionBase
    {
        [SerializeField]
        ...
        {
            if (ShootSelf)
                _object = entity.gameObject;
            if (_object == null)
            {
                Debug.LogWarning("No object to shoot");
                _isRunning = false;
                return;
            }
            ...
            IComponentRepository repo = _object.GetComponent<IComponentRepository>();
            if (repo != null)
            {
                ...
                repo.SendMessageToComponents<AnimationComponent>(0f, string.Empty, Telegrams.SetFacingDirection,
                    direction);
            }
            _isRunning = false;
        }
        ...
        private Vector2 CalculateVelocity()
        {
            ...
            switch (_shootTarget)
            {
                ...
                case ShootTarget.ToTarget:
                    ...
                    break;
                case ShootTarget.ToParentFacingDirection:
                    var parent = _aIComponent.Parent.GetComponent<IComponentRepository>();
                    var parentAnimComponents = parent.Components.GetComponents<AnimationComponent>();
                    for (int i = 0; i < parentAnimComponents.Count; i++)
                    {
                        var animComponent = parentAnimComponents[i] as AnimationComponent;
                        if (animComponent.IsMain())
                        {
                            velocity = animComponent.GetFacingDirectionVector();
                            break;
                        }
                    }
